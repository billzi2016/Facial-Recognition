...
- 使用 CPU 半数 worker 做读图、解码、预处理和任务排队。
...
- CPU loader worker 数量固定为 `max(1, os.cpu_count() // 2)`。
- loader worker 只做图片路径读取、图片解码、颜色格式转换、基础校验和 batch 组装。
- 模型推理端使用单个受控推理进程或小队列，不无节制多进程同时持有模型。
